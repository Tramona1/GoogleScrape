# proxy_test_crawler.py - Corrected Proxy Test Script (Forced SSL Off & HTTP)
...
# --- Copy get_proxy_pool() function from your utils.py (UPDATED PORTS) ---
BRIGHTDATA_PORTS = [22225, 22226, 33335]  # Added fallback port - FALLBACK PORT ADDED HERE
...
async def test_proxy(proxy_url, proxy_auth, disable_ssl_verify=True):  # Changed default to True - SSL OFF BY DEFAULT for test
    """Tests a proxy connection using aiohttp."""
    try:
        ssl_context = False if disable_ssl_verify else None  # Control SSL verification based on parameter
        async with aiohttp.ClientSession(
            connector=aiohttp.TCPConnector(ssl=ssl_context)  # Disable SSL verification based on ssl_context
        ) as session:
            test_url = "http://api.ipify.org?format=json"  # HTTP Test URL - HTTP URL FOR TEST
            async with session.get(
                test_url, # HTTP Test URL - HTTP URL FOR TEST
                proxy=proxy_url,
                proxy_auth=proxy_auth,
                timeout=30  # Increased timeout - IMPORTANT for potentially slower proxies
            ) as response:
                if response.status == 200:
                    json_response = await response.json()
                    print(f"Proxy Test Successful!")
                    print(f"Response from {proxy_url}:")
                    print(json_response)
                else:
                    print(f"Proxy Test Failed! Status Code: {response.status} for {proxy_url}")
                    response_text = await response.text()
                    print(f"Response Text: {response_text}")
    except aiohttp.ClientError as e:
        print(f"aiohttp ClientError for {proxy_url}: {e}")
